...
.\setup.ps1 -DatabaseName calculator -Password 7297

# 3. Start the application
.\start.ps1 -Password 7297
```
...
From command line:
```bash
PGPASSWORD=7297 psql -h localhost -U postgres -c "CREATE DATABASE calculator;"
```

Or from WSL connecting to Windows PostgreSQL:
```bash
PGPASSWORD=7297 psql -h 10.255.255.254 -U postgres -c "CREATE DATABASE calculator;"
```
...
```bash
PGPASSWORD=7297 psql -h localhost -U postgres -d calculator -f /path/to/clonetemplate/server/infrastructure.sql
```
